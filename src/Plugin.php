...
class Plugin
{
     ...
     * @param Plugin $obj
     */
    public static function hooks(Plugin $obj)
    {
    ...
    public function createMarkdownLink($post)
    {
        ...
        if ($use_markdown) {
            ...
            <div class="misc-pub-section">
                ...
                <a href="javascript:{}"
                onclick="document.getElementById('<?php echo esc_attr(self::METAKEY); ?>').value = 0; document.getElementById('post').submit(); return false;"><?php _e('Disable'); ?></a>
            </div>
            <?php
        } else {
            ?>
            <div class="misc-pub-section">
                <span class="dashicons dashicons-editor-code"></span> Markdown:
                <a href="javascript:{}"
                onclick="document.getElementById('<?php echo esc_attr(self::METAKEY); ?>').value = 1; document.getElementById('post').submit(); return false;"><?php _e('Enable'); ?></a>
            </div>
            <?php
        }
    }
    ...
    public function saveMarkdownMeta($post_id, $post)
    {
        ...
        if (! isset($_POST[self::NONCE]) || ! wp_verify_nonce(sanitize_text_field(wp_unslash($_POST[self::NONCE])), $post_id)) {
            return;
        }
        ...
        $use_markdown = ! empty($_POST[self::METAKEY]) ? (int) sanitize_text_field(wp_unslash($_POST[self::METAKEY])) : 0;
    ...
    /**
     * Check if the block editor (Gutenberg) is being used for a post.
     * This works for vanilla WordPress, Classic Editor plugin, Pressbooks, etc.
     *
    ...
    {
        // Check for block markers on front-end content (outside admin context)
        if (! is_admin()) {
            $post_object = get_post($post);
            if ($post_object && has_blocks($post_object->post_content)) {
                return true; // Has block markers, content was created with Gutenberg
            }

            return false; // No block content = Classic Editor content
        }

        // Use WordPress's block editor check for admin pages
